...
function displaySearchUsageResult(message) {
    ...
    if (message.length === 0) {
        ...
        message.forEach(uses => {
            var listItem = document.createElement('li');
            listItem.textContent = uses.userName + ': (DEVICE_NAME)' + uses.deviceName + ' (USAGE_DATE)' + uses.usageDate + ' (USAGE_DURATION)' + uses.usageDuration; // Update property names according to the backend
